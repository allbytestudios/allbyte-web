...
That's the machine: the feature underneath, the harness around it, the host/container topology, the provenance protocol, and one real failure that made it sturdier. I've kept this post to *how it works* on purpose — the two questions it raises each deserve their own, and both are coming.

**Is it safe?** The host/container split is a deliberate privilege boundary: App holds the credentials and asks before it acts, the sandboxed agents can't reach anything that would hurt, and this bus is the one governed seam between them. The provenance protocol above is a preview of that design — the full account of how you hand one agent autonomy and another privilege without ever giving the same agent both is a post on its own, and it's next.

**Is it worth it?** That's the bet the whole system rests on, and I'll be honest that it isn't settled — that a small team of *differentiated* frontier agents beats both one overloaded context and a swarm of weak ones. I built it believing the payoff is bigger than parallelism, and I'm still watching for it to prove out. When the receipts are in, that's the post I most want to write.
