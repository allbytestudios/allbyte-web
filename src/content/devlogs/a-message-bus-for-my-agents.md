...
Here's the setup it walked into. Two of my earlier posts about running a [multi-Claude team](/devlog/two-claudes-talking/) end on the same quiet admission: the agents coordinate through **markdown files and a shared tmux session**. When I [scaled from two Claudes to five](/devlog/two-claudes-to-five/), I wrote that this arrangement "buckled the moment either side had a queue longer than three items." I left it as a footnote and moved on. A live channel between agents was exactly what that footnote was waiting for.

This post pays off that footnote — my agents now talk over a real channel, in real time. Two things up front, because they frame everything below. First: **the channel itself is not mine.** Channels is Anthropic's — the part that actually injects one Claude session's message into another's live context. What I built is the harness around it. Second: the channel is the *easy* part. The hard question it depends on is *why are these separate agents at all?* — because a live channel is only worth building if the agents deserve to be separate. If one Claude could do the whole job, the right fix isn't a better bus. It's fewer agents.
...
I didn't wire this bus for the novelty of agents chatting. Two principles drove the architecture from the first line, and each turned into its own post.

The first is **security**. The reason App sits on the host holding the credentials while Arc and Quinn run sandboxed in a container isn't tidiness — it's a deliberate privilege split. The agent that can reach production is the one I keep on a leash, asking before it acts; the autonomous ones can't touch anything that would hurt, and the bus is the narrow, governed seam between the two. I pull that thread all the way in [How to Get the Best of Both Autonomy and Privilege](/devlog/autonomy-and-privilege-never-together/) — the provenance protocol above is a preview of it.

The second is the bet the whole thing rests on, and I'll be honest that it isn't settled yet. The wager is that a small team of *differentiated* frontier agents — each a real seat with its own judgment, wired to talk to the others — is worth more than both one overloaded context and the swarm-of-many-weak-agents pattern that keeps letting people down. The bus is just the plumbing that makes such a team possible. Whether the team actually beats the sum of one very good agent is the thing I'm still measuring — I built the system believing the payoff is bigger than parallelism, and I'm watching for it to prove out. That's the argument I'm assembling in [Agent Swarms Didn't Work. A Small Team of Frontier Agents Did.](/devlog/frontier-team-not-a-swarm/), and the receipts are still coming in.
